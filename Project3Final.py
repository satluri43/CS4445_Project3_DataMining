...
def visualize_dbscan(X, labels, stroke=None, title="", filename=""):
    ...
    for i, lbl in enumerate(np.unique(labels)):
        ...
        if lbl == -1:
            plt.scatter(
                X_2d[mask, 0], X_2d[mask, 1], c="black", s=10, label="noise", alpha=0.6
            )
        else:
            plt.scatter(
                X_2d[mask, 0],
...
def run_dbscan_experiment(name, X, eps, min_samples=5, stroke=None):
    ...
    runtime = t1 - t0
    print(
        f"Runtime: {runtime:.3f}s  |  Noise: {noise} ({noise / len(labels) * 100:.2f}%)"
    )
...
plt.tight_layout()
plt.savefig(
    os.path.join(PLOTS_DIR, "kmeans_elbow_silhouette.png"), dpi=150, bbox_inches="tight"
)
plt.close()
...
print(
    df_viz.groupby("km_label")[cont_cols + ["hypertension", "heart_disease"]]
    .mean()
    .round(3)
)
...
plt.tight_layout()
plt.savefig(
    os.path.join(PLOTS_DIR, "silhouette_kmeans.png"), dpi=150, bbox_inches="tight"
)
plt.close()
...
plt.tight_layout()
plt.savefig(
    os.path.join(PLOTS_DIR, "kmeans_pca_scatter.png"), dpi=150, bbox_inches="tight"
)
plt.close()
print("Saved: kmeans_pca_scatter.png")
print(
    pd.DataFrame(kmeans_pca_results)[["k", "SSE", "Silhouette"]].to_string(index=False)
)
...
for method in linkage_methods:
    ...
    print(
        f"  {method:<12} | {sse(X, lbl):>10.2f} | {silhouette_score(X, lbl):>10.4f} | {sizes}"
    )
...
labels_e1 = run_dbscan_experiment(
    "Experiment 1 (eps=1.75)", X_exp, eps=1.75, stroke=y_exp
)

# Experiment 2 – Stroke-only clustering
stroke_only = data_exp_sc[data_exp_sc["stroke"] == 1].drop(columns=["stroke"])
labels_e2 = run_dbscan_experiment(
    "Experiment 2 (stroke only)", stroke_only.values, eps=1.3
)
...
for group_name in ["young", "middle", "old"]:
    ...
    run_dbscan_experiment(
        f"Experiment 3 ({group_name})", X_group, eps=1.4, stroke=y_group
    )

# Experiment 4 – Feature reduction + composite smoking score
ex4 = data_exp_sc.drop(
...
labels_e4 = run_dbscan_experiment(
    "Experiment 4 (feature reduction)", ex4.values, eps=1.3, stroke=y_exp
)

# Experiment 5 – Weighted hypertension & heart disease
ex5 = data_exp_sc.drop(columns=["stroke"]).copy()
ex5[["hypertension", "heart_disease"]] *= 3
labels_e5 = run_dbscan_experiment(
    "Experiment 5 (weighted features)", ex5.values, eps=1.6, stroke=y_exp
)
...
ward_dist = np.array(
    [np.linalg.norm(X[i] - ward_cent[ward_lbl[i]]) for i in range(len(X))]
)
...
axes[0].axvline(
    km_thresh, color="red", linestyle="--", label=f"97th pct={km_thresh:.2f}"
)
...
axes[1].axvline(
    ward_thresh, color="red", linestyle="--", label=f"97th pct={ward_thresh:.2f}"
)
...
plt.tight_layout()
plt.savefig(
    os.path.join(PLOTS_DIR, "contingency_matrices.png"), dpi=150, bbox_inches="tight"
)
plt.close()
...
print("  Running MDS …")
X_mds = MDS(
    n_components=2, random_state=RANDOM_STATE, dissimilarity="euclidean"
).fit_transform(X_vis)

print("  Running t-SNE …")
X_tsne = TSNE(n_components=2, random_state=RANDOM_STATE, perplexity=30).fit_transform(
    X_vis
)

print("  Running UMAP …")
X_umap = UMAP(
    n_components=2, random_state=RANDOM_STATE, n_neighbors=20, min_dist=0.1
).fit_transform(X_vis)
...
plt.tight_layout()
plt.savefig(
    os.path.join(PLOTS_DIR, "cluster_visualizations.png"), dpi=150, bbox_inches="tight"
)
plt.close()
